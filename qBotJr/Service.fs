...
///original module for testing, experimenting with the Discord.NET API
module stuff =
  ...
  let messageReceivedAsync (msg: SocketMessage) =
    ...
    | "QPING" ->
        Async.AwaitTask
          (msg.Channel.SendMessageAsync (@"<@!442438729207119892> <@!643435344355917854> <@!257947314625314816>"))
        |> ignore
        ()
    | "QHERE" ->
        let x =
          Async.AwaitTask (msg.Channel.SendMessageAsync ("blah blah blah\n\n👌\tOk\n❌\tCancel \n👈\tBack \n🖕\tFuck Off"))
        ...
        |> Seq.iter (fun x ->
             ...
             Thread.Sleep 700
             Async.AwaitTask (msg2.AddReactionAsync (new Emoji(x))) |> ignore)
    | "QTEST" ->
        ...
        guild.Users
        |> Seq.iteri (fun i (x: SocketGuildUser) ->
             sprintf "%i\t%s - %s - %s" i x.Username x.Nickname (x.Status.ToString ()) |> logger.WriteLine)
